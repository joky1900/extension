...
GazeDataCollection = {
    ...
    gazeListener: function (data) {
        if (data === null) {
            return;
        }

        let gaze = {};
        let currentTime = performance.now() - GazeDataCollection.startTime; // calculate elapsed time since start
        gaze["timeStamp"] = currentTime;
        console.log("clock: " + currentTime);
        // Round the coordinates as too many fractional pixels
        gaze["x"] = Util.roundToTwo(data.x);
        gaze["y"] = Util.roundToTwo(data.y);
